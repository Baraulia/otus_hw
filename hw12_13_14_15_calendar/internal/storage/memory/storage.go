...
func New(logger app.Logger) *Storage {
	...
	return &Storage{repository: repo, logger: logger, mu: mu}
}

func (s *Storage) CreateEvent(ctx context.Context, eventDTO models.Event) (string, error) {
	newUUID := uuid.New()
	s.mu.Lock()
...
func (s *Storage) UpdateEvent(ctx context.Context, eventDTO models.Event) error {
	if eventDTO.ID == "" {
...
func (s *Storage) GetListEventsDuringDay(ctx context.Context, targetDay time.Time) ([]models.Event, error) {
...
func (s *Storage) GetListEventsDuringWeek(ctx context.Context, start time.Time) ([]models.Event, error) {
	events := make([]models.Event, 0)
	s.mu.RLock()
	for id, event := range s.repository {
		startDay := start.Truncate(24 * time.Hour)
		finishDay := startDay.Add(24 * 8 * time.Hour).Truncate(24 * time.Hour)
...
func (s *Storage) GetListEventsDuringMonth(ctx context.Context, start time.Time) ([]models.Event, error) {
	events := make([]models.Event, 0)
	s.mu.RLock()
	for id, event := range s.repository {
		startDay := start.Truncate(24 * time.Hour)
		finishDay := startDay.Add(24 * 31 * time.Hour).Truncate(24 * time.Hour)
